	...
	if($ftp){
		...
		if ($path && !file_exists($path))
		{
			if (mkdir($path, $permission, true))
			{
				...
				@fwrite($path.'/index.html','<html><head><title>403 Forbidden</title></head><body><p>Directory access is forbidden.</p></body></html>');
			}
		} // or even 01777 so you get the sticky bit set
		if ($path_thumbs)
		{
			if (mkdir($path_thumbs, $permission, true))
			{
				// create file index.html
				@fopen($path_thumbs.'/index.html', 'w');
				@fwrite($path_thumbs.'/index.html','<html><head><title>403 Forbidden</title></head><body><p>Directory access is forbidden.</p></body></html>');
			}
		} // or even 01777 so you get the sticky bit set
		umask($oldumask);
		return true;
