...
class Theme extends Backend_Controller {
	...
	private function _create_blank_theme()
	{
		    ...
		    )
		{
			...
			if ( $this->form_validation->run() )
			{
				...
				@mkdir(PUBLICPATH.'themes/'.$folder, 0777, true);
				@fopen(PUBLICPATH."themes/$folder/index.html", "w");
	...
	public function delete_theme()
	{
		if ( $this->input->is_ajax_request() && $this->role->i('delete') ) 
		{
			...
			if ( $this->theme_model->delete($idTheme) )
			{
			    ...
			    @delete_folder(PUBLICPATH.'themes/'.$folderTheme); // delete theme asset dir.
		...
		if ( $this->role->i('write') )
		{
			if ( $this->input->method() == 'post' )
			{
				...
				else
				{
					...
					{
						// Extract zip file.
						$unzip_path = PUBLICPATH."temp/$zip_name";
						$zip_file   = PUBLICPATH."temp/$zip_name.zip";
						...
						$view_path = PUBLICPATH."temp/$zip_name/views/";
						@copy_folder($view_path,  $view_destination_path);

						// Create assets dir
						$assets_destination_path = PUBLICPATH."themes/$theme_folder";
						@mkdir($assets_destination_path);
						
						// copy assets temp to content themes.
						$assets_path = PUBLICPATH."temp/$zip_name/assets/";
		...
		if ( $this->role->i('modify') )
		{
			...
			if ( !empty($this->vars['res_theme']) ) 
			{
				...
				elseif ( $this->_act == 'upload_theme_assets' )
				{
					...
					{
						$zip_path = PUBLICPATH."temp/$zip_name.zip";
						$destination_path = PUBLICPATH."themes/$folder_edit/";
	...
	private function _runBackup($theme)
	{
		...
		$path_assets = PUBLICPATH.'themes/'.$theme['folder'];
		$dirname     = 'themez'.md5($theme['folder']);
		$zip_name    = $dirname.'.zip';
		$dir_temp    = PUBLICPATH.'temp/'.$dirname;
		...
		if (mkdir($dir_temp))
		{
			...
			if (rename(PUBLICPATH.'temp/'.$zip_name, PUBLICPATH.'uploads/'.$zip_name))
			{
