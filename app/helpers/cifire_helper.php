...
if ( ! function_exists('content_urlX'))
{
	/**
	 ...
	 * @return  string  
	*/
	function content_urlX($param = '', $protocol = NULL)
	{
		$CI =& get_instance();
		return $CI->config->base_url($param, $protocol);
	}
}
...
if ( ! function_exists('favicon'))
{
	...
	*/
	function favicon($param = '') 
	{
		if ($param == 'logo')
		{
			$result = site_url('uploads/'.get_setting('web_logo'));
		}
		elseif ($param == 'web_image')
		{
			$result = site_url('uploads/'.get_setting('web_image'));
		}
		else
		{
			$result = site_url('uploads/'.get_setting('favicon'));
		}
	...
	*/
	function post_images($filename = null, $mode = NULL, $noimage = FALSE)
	{
		...
		switch ($noimage) 
		{
				...
				if ( !empty($filename) )
				{
					if ( $mode == 'thumb' ) 
					{
						if (file_exists(PUBLICPATH."thumbs/$filename")) 
							$image_url = site_url("thumbs/$filename").$dt;
						else 
							$image_url = site_url("images/thumb_noimage.jpg").$dt;
					}
					elseif ( $mode == 'medium' ) 
					{
						if (file_exists(PUBLICPATH."uploads/medium/$filename")) 
							$image_url = site_url("uploads/medium/$filename").$dt;
						else
							$image_url = site_url("images/medium_noimage.jpg").$dt;
					}
					else
					{
						if ( file_exists(PUBLICPATH."uploads/$filename") ) 
							$image_url = site_url("uploads/$filename").$dt;
						else
							$image_url = site_url("images/noimage.jpg").$dt;
					}
				}
			...
			default:
			case FALSE:
				if ( !empty($filename) && file_exists(PUBLICPATH."uploads/$filename") && $mode == '' ) 
					$image_url = site_url("uploads/$filename").$dt;
				elseif ( !empty($filename) && file_exists(PUBLICPATH."uploads/medium/$filename") && $mode == 'medium' ) 
					$image_url = site_url("uploads/medium/$filename").$dt;
				elseif ( !empty($filename) && file_exists(PUBLICPATH."thumbs/$filename") && $mode == 'thumb' ) 
					$image_url = site_url("thumbs/$filename").$dt;
				else
					$image_url = '';
			break;
...
if ( ! function_exists('post_file'))
{
	...
	*/
	...
	{
		$url_file = '';
		if ( file_exists(PUBLICPATH . "uploads/$type/$filename") )
			$url_file = site_url("uploads/$type/$filename");
